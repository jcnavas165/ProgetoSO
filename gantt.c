 ...
 *
 * Percorre o histórico da tarefa para encontrar qual era seu estado
 * no tick dado. Usa o último evento cujo tick <= tick_query.
 *
 * Retorno:
 *   Estado da tarefa (-1 se ainda não havia chegado)
 *   cpu_out: CPU onde executava (-1 se não estava rodando)
 *   event_out: tipo do evento neste tick
 */
 ...
 *
 * SVG aceita cores como "#RRGGBB". Adicionamos o '#' se necessário.
 * Escrevemos o resultado em 'out' (deve ter >= 8 bytes).
 */
 ...
 *   1. Cabeçalho SVG (dimensões, estilos)
 *   2. Título do gráfico
 *   3. Grade de fundo (linhas de grade)
 *   4. Para cada tarefa (de baixo para cima, ID menor = mais baixo):
 *      - Label com ID da tarefa
 *      - Bloco colorido para cada tick que a tarefa executou
 *      - Bloco branco para cada tick que aguardou (READY)
 *      - Bloco preto para cada tick suspensa
 *      - Ícone de chegada (▶) e término (■)
 *   5. Eixo X com marcações de tempo
 *   6. Labels das CPUs nos blocos
 *   7. Legenda
 *
 * Decisão de ordenação no eixo Y (requisito 2.5):
 *   "A tarefa com ID menor é a mais próxima do eixo X"
 *   Como desenhamos de cima para baixo, a última tarefa (ID maior) é a
 *   mais próxima do topo do SVG, e a primeira (ID menor) fica na parte
 *   mais baixa antes do eixo X.
 */
int gantt_generate_svg(const SimState *sim, const char *filename) {
     ...
     *   Largura = margem_esquerda + (ticks * largura_célula) + margem_direita
     *   Altura  = margem_topo + (tarefas * altura_célula) + margem_base
     */
    ...
    /* ── Fundo branco para a área do gráfico ───────────────────────────── */
    ...
    /* ── Desenha as tarefas ─────────────────────────────────────────────── */
    /*
     * Ordem no eixo Y (requisito 2.5):
     *   ID menor = mais próximo do eixo X = linha MAIS BAIXA no SVG
     *   Portanto, desenhamos em ordem DECRESCENTE de índice.
     *
     * Para a tarefa com índice i (0-based, ordenado por ID):
     *   row = num_tasks - 1 - i  (i=0 → row mais baixa; i=n-1 → row mais alta)
     *
     * Como o SVG tem y crescendo para baixo:
     *   y_topo = GANTT_MARGIN_T + row * GANTT_CELL_H
     *          = GANTT_MARGIN_T + (num_tasks - 1 - i) * GANTT_CELL_H
     */

    /* Encontra o índice das tarefas ordenadas por ID crescente */
    ...
    for (int oi = 0; oi < num_tasks; oi++) {
        int idx = order[oi]; /* Índice real no array de tarefas */
        ...
        /* ── Desenha um bloco para cada tick ──────────────────────────── */
        ...
        /* Linha de chegada: linha vertical verde no tick de arrival */
        if (t->arrival < num_ticks) {
            int xa = GANTT_MARGIN_L + t->arrival * GANTT_CELL_W;
            svg_line(f, xa, y, xa, y + GANTT_CELL_H, "#00CC00");
        }
        /* Linha de término: linha vertical vermelha no tick de finish */
 ...
 *
 * Caracteres usados:
 *   [#] = executando  (bloco cheio)
 *   [.] = pronto/aguardando
 *   [X] = suspensa
 *   [ ] = fora do sistema (antes de chegar ou após terminar)
 *
 * Cada coluna representa um tick. As tarefas aparecem com ID menor embaixo
 * e ID maior em cima (como no gráfico SVG).
 */
